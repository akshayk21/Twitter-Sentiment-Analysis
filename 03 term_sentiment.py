    ...
    for word in list: 
        ...
        if word in sentimentScores.keys():
            ...
            if word not in newWords:
                newWords[word] = [finalScore]
            else:
                newWords[word].append(finalScore)


    for term, score in newWords.items():
        print(term, "\t", sum(score)/len(score))
    
    
def getTweets(file, sent_scores):
    #This functions returns all the tweet text collected 
    
    print("\nCalculated score of each term not present in the ASINN-111.txt file")
